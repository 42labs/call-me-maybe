...
MOCK_ORACLE_CONTRACT_FILE = os.path.join(os.path.dirname(__file__), "MockOracle.cairo")
...
async def contracts(private_keys):
    ...
    await token_contract.transfer(
        buyer_account_contract.contract_address, transfer_amount
    ).invoke(caller_address=seller_account_contract.contract_address)

    return (
        call_option_contract,
...
async def test_deploy(contracts):
    ...
    result = await token_contract.balanceOf(
...
async def test_call(contracts):
    ...
    strike_price = to_uint_decimals(
        21
    )  # Half of 42, the oracle's answer so buyer and seller should end up with half of the collateral
    currency_address = token_contract.contract_address
    oracle_key = str_to_felt(
        "tst/usd"
    )  # Doesn't matter because mock oracle always returns 42
    ...
    await token_contract.approve(call_option_contract.contract_address, fee).invoke(
        caller_address=buyer_account_contract.contract_address
    )
    await call_option_contract.register_call_option(call_option_submission).invoke(
        caller_address=buyer_account_contract.contract_address
    )

    await token_contract.approve(call_option_contract.contract_address, size).invoke(
